...
if [[ -f "$recording_pid_file" ]]; then
  ...
  (
    set +e
    "${cmd[@]}" > /tmp/msnap-cast.log 2>&1
    rc=$?
    if [[ -f "$recording_pid_file" ]]; then
      rm -f "$recording_pid_file" "$recording_filepath_file" "/tmp/msnap-cast.starttime"
      if [[ $rc -ne 0 ]]; then
        notify_error "Recording lost" \
          "gpu-screen-recorder crashed. See <i>/tmp/msnap-cast.log</i>."
      fi
    fi
  ) &
